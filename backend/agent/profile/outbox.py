...
async def sync_pending(vector_store: Any, *, limit: int = 50) -> dict[str, int]:
    ...
    with connect() as db:
        rows = db.execute("SELECT id,entity_type,entity_id,collection FROM vector_sync_outbox WHERE status='pending' AND available_at<=now() ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT %s", (max(1,min(limit,500)),)).fetchall()
        for row in rows:
    ...
    for row in rows:
        try:
            with connect() as db:
                entity = None
                if row['entity_type'] == 'avatar_memory': entity = db.execute("SELECT * FROM avatar_memories WHERE id=%s", (row['entity_id'],)).fetchone(); await index_memory(entity, vector_store, collection=row['collection'])
            ...
            with connect() as db:
                db.execute("UPDATE vector_sync_outbox SET status='failed',last_error=%s WHERE id=%s", (str(exc)[:1000],row['id'])); failed += 1
