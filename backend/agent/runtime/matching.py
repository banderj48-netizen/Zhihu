...
class AvatarMatcher:
    ...
    def __init__(self, candidate_provider: Callable[[str], Awaitable[list[dict[str, Any]]]], llm: LLM | None = None) -> None:
        """注入场景候选读取函数和可选匹配模型。"""
    ...
    async def match(self, avatar_id: str, scene_id: str, mode: Literal["llm", "random", "manual"], target_avatar_id: str | None = None) -> dict[str, Any]:
        """按照指定模式返回一个空闲候选。"""
        candidates = [x for x in await self._provider(scene_id) if str(x.get("avatar_id")) != avatar_id and x.get("status", "idle") == "idle"]
        if mode == "manual":
